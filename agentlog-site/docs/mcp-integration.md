...
AgentLog 通过 **Model Context Protocol (MCP)** 与各类 AI Agent 集成，实现 AI 交互的主动上报。本文档详细说明如何为不同的 AI 工具配置 MCP 连接。
...
| 配置方式 | 适用场景 | 操作复杂度 |
|----------|----------|------------|
| **自动配置向导**（推荐） | VS Code 插件用户，配置 Cline、OpenCode 等 | 简单（图形界面） |
| **手动配置文件** | 高级用户，自定义 MCP 客户端 | 中等（编辑 JSON） |
