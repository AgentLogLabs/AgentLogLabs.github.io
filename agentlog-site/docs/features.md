...
**场景**：多个 AI Agent 可以同时在同一仓库的不同 worktree 上独立工作，各自会话精准绑定到对应的 Commit，互不干扰。
...
1. **`gitRepoRoot` 字段**：新增字段存储 Git 仓库根目录路径。无论会话发生在哪个 worktree，同一仓库下的所有会话共享相同的 `gitRepoRoot`。

2. **动态钩子脚本**：post-commit 钩子通过 `git rev-parse --show-toplevel` 动态获取当前 worktree 的真实路径，一次安装，所有 worktree 均生效。

3. **三级匹配策略**：post-commit 触发时按以下顺序查找未绑定会话：
...
4. **会话自动推断**：创建 Session 时后端自动通过 `git rev-parse --show-toplevel` 填充 `gitRepoRoot`。

**使用方式**：只需在**任意一个** worktree（通常是主仓库）安装 Git 钩子，所有 worktree 均自动支持。
